...
响应同时给出完成比例、下一步、已配置/已验证目标数、首次价值时间、诊断证据来源数和从激活到首次价值的分钟数。`POST /settings/targets` 会保存目标并立即探测；失败目标保留为待修复配置，但不会完成接入里程碑，可通过 `POST /settings/targets/{name}/verify` 重试。仅发起查询、进入澄清流程或对不存在的服务执行无证据排障也不会完成后续里程碑。前端直接展示该清单，不依赖浏览器本地标记来伪造完成状态。
...
- 邀请实例数、领取率和领取失败率。
- 已验证目标接入率、首次查询率、首次诊断率；不可达配置单独统计，不能算作接入成功。
